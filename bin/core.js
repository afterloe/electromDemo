 ...
 */
    (userId, callback) => {
        ...
        if (!fs.existsSync(__dbPath)) {
            let readStream = fs.createReadStream("./app.db"),
 ...
 */
    xlsx => {
        let db = new Object(), _tableName, builder;
        xlsx.forEach(_table => {
            _tableName = _table.name;
            builder = `${_tableName}Builder`;
            if(InnoDB[builder])
                db[_tableName] = InnoDB[builder].apply(InnoDB,[_table.data]);
        });
        return db;
    }
];
...
const ConfigSecurity = require("../domain/confSecurity")
    , configuer = new ConfigSecurity("885d081b-7b5f558e-dfb2ca66-f498605d");

module.exports.configuer = configuer;
